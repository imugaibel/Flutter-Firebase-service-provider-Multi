...
import 'package:omni/widgets/alert.dart';
...
class _ServiceFormState extends State<ServiceForm> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        key: _scaffoldKey,
        body: (widget.uidService == null || widget.uidService == "")
            ? _formAdd()
            : _formEdit());
  }
  ...
  _btnSubmit({String uid = ""}) {
    if (_images.length == 0) {
      _scaffoldKey.showTosta(
          message: AppLocalization.of(context)
              .translate("Please enter photos of the service"),
          isError: true);
      return;
    }
    ...
    if (!_validate()) {
      _scaffoldKey.showTosta(
          message:
              AppLocalization.of(context).translate("Please fill all fields"),
          isError: true);
      return;
    }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                          labelText:
                              AppLocalization.of(context).translate("Info EN")),
                    ),
                    SizedBox(
                        height:
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                          labelText:
                              AppLocalization.of(context).translate("Info AR")),
                    ),
                    SizedBox(height: 10),
                    TextFormField(
                      onSaved: (value) => cityEN = value.trim(),
                      textInputAction: TextInputAction.next,
                      decoration: InputDecoration(
                          labelText:
                              AppLocalization.of(context).translate("City EN")),
                    ),
                    SizedBox(height: 10),
                    TextFormField(
                      onSaved: (value) => cityAR = value.trim(),
                      textInputAction: TextInputAction.done,
                      decoration: InputDecoration(
                          labelText:
                              AppLocalization.of(context).translate("City AR")),
                    ),
                    SizedBox(
                        height:
                            MediaQuery.of(context).size.height * (90 / 812)),
                    BtnMain(
                        title: widget.uidService == null
                            ? AppLocalization.of(context)
                                .translate("Add Service")
                            : AppLocalization.of(context)
                                .translate("Edit Service"),
                        onTap: () => _btnSubmit(uid: widget.uidService)),
                  ],
                ),
              ),
            )
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            if (!isGetData) {
              ...
              cityEN = service.cityEN;
              cityAR = service.cityAR;
              isGetData = true;
            }
            ...
            return CustomScrollView(
              slivers: <Widget>[
                SliverAppBar(
                  ...
                  expandedHeight:
                      MediaQuery.of(context).size.height * (350 / 812),
                  floating: true,
                  pinned: true,
                  snap: true,
                  backgroundColor: Colors.transparent,
                            ...
                            BtnMain(
                                title: widget.uidService == null
                                    ...
                                    : AppLocalization.of(context)
                                        .translate("Edit Service"),
                                onTap: () =>
                                    _btnSubmit(uid: widget.uidService)),
                          ],
                        ),
                      ),
                    )
